...
class GiaoVienController extends Controller
{
    ...
    function storepc(Request $req){
        ...
        if($edit != null && count($edit) > 0){
            DB::table('phancongs')
                ->where('id_giaovien', $id_giaovien)
                ->update(['id_lophoc'=>$id_lop,'id_monhoc'=>$id_mon,'ca_day'=>$ca_day]);
        ...
        if($edit2 != null && count($edit2) > 0){
            DB::table('phancongs')
                ->where('id_lophoc', $id_lop)
                ->where('id_monhoc', $id_mon)
                ->update(['id_giaovien'=>$id_giaovien,'ca_day'=>$ca_day]);
            return redirect("admin/giaovien/phancong");
        }
        // Thêm bản ghi mới
        $rs = PhanCong::save($id_giaovien,$id_lop,$id_mon,$ca_day);
        if($rs == true){
            return redirect("admin/giaovien/phancong");
        }
        else{
    ...
    function updatepc(Request $req,$id_giaovien){
        ...
        $ca_day = $t2.$t3.$t4.$t5.$t6.$t7;
        $rs = PhanCong::update($id_giaovien,$id_lophoc,$id_monhoc,$ca_day);
        if($rs == true){
            return redirect("admin/giaovien/phancong");
        }
        else{
